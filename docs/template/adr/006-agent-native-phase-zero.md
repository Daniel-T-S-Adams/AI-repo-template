...
1. a repository is created from `Daniel-T-S-Adams/AI-repo-template`;
...
4. the agent should organize and publish the real project without requiring the user to explain how `AI-repo-template` works.
...
- **source-template mode** — repository is `Daniel-T-S-Adams/AI-repo-template`; and
...
Rejected as the sole solution because `AI-repo-template` is also a maintained public project with real security, validation, documentation, and release needs. Agent entry behavior can separate source-template concerns from downstream project concerns without discarding all source capabilities.
