...
- references to `Daniel-T-S-Adams/AI-repo-template`, old `/project:init-template`, template badges, template workflows, or historical template paths.
...
- a first commit/tree closely matching a historical `AI-repo-template` snapshot;
...
- explicit references to `Daniel-T-S-Adams/AI-repo-template`.
...
- **C — Current template:** the current `Daniel-T-S-Adams/AI-repo-template` baseline and its downstream-relevant changes.
...
Treat `AI-repo-template` as an upstream knowledge source, not a filesystem authority. Selectively adopt high-value improvements. Do not replace the project's README, architecture, application CI, agent instructions, or source structure with current template versions.
...
- retain `source_repository: Daniel-T-S-Adams/AI-repo-template`;
...
Bump the baseline when a future `AI-repo-template` change materially alters downstream bootstrap, agent-routing, security/governance expectations, provenance, or reconciliation behavior. Routine source-template maintenance that does not change downstream compatibility does not require a new baseline.
