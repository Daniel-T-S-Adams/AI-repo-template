...
> Canonical SOP for the first Claude Code or Codex agent entering a repository created from `Daniel-T-S-Adams/AI-repo-template`. Phase 0 prepares the repository substrate before project-specific implementation, while immediately ingesting real project material when it is already available.
...
- the repository is **not** `Daniel-T-S-Adams/AI-repo-template` itself;
- the repository still contains the generic `AI-repo-template` README and/or generic root agent instructions;
...
> [!CAUTION]
> Never run destructive Phase 0 cleanup against the canonical `Daniel-T-S-Adams/AI-repo-template` source repository.
...
- documentation describing how to choose or create a repo from `AI-repo-template`.
...
- `AI-repo-template` title/marketing in the root README;
...
- Can a future agent tell how to handle a later `AI-repo-template` compatibility upgrade without rerunning Phase 0?
